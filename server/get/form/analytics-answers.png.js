...
module.exports = {
    path: '/form/analytics-answers.png',
    async run(app, req, res) {
        try {
            ...
            // Check if logs exist
            if (form.answers.length <= 0 || form.answers == null) {
            ...
            form.answers.forEach((answer) => {
                currentAnswers++;
                const time = moment(parseFloat(answer.date)).tz(req.user.timezone).format('YYYY / MMM / HH:mm')
                if (xLabels.indexOf(time, -1) >= 0) {
                    yLabels[xLabels.indexOf(time, -1)] = yLabels[xLabels.indexOf(time, -1)] + 1;
                } else {
                    yLabels.push(currentAnswers);
                    xLabels.push(time)
                }
            })
            ...
            const configuration = {
                ...
                data: {
                    ...
                    datasets: [
                        {
                            label: `Number of answers over time`,
                            data: yLabels,
                            fill: true,
